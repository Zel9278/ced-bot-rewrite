window.diff = (() => {
    ...
    function getSolution(
        ...
        if (d === 0) {
            ...
            const removeFirst =
                (startDirect & Direct.horizontal ? 1 : 0) +
                (endDirect & Direct.vertical ? 1 : 0)
            const addFirst =
                (startDirect & Direct.vertical ? 1 : 0) +
                (endDirect & Direct.horizontal ? 1 : 0)
        ...
        for (let y = 0; y <= HALF; ++y) {
            ...
            for (let k = -d; k <= d; ++k) {
                ...
                const currentPoints = [
                    {
                        ...
                        segments:
                            now[k - 1].segments +
                            (now[k - 1].direct & Direct.horizontal ? 0 : 1),
                    },
                    {
                        direct: Direct.vertical,
                        d: preview[k + 1].d + 1,
                        segments:
                            preview[k + 1].segments +
                            (preview[k + 1].direct & Direct.vertical ? 0 : 1),
                    },
                ]
                ...
                if (
                    ...
                    y > 0 &&
                    y <= N &&
                    compareFunc(a[aStart + x - 1], b[bStart + y - 1])
                ) {
                    currentPoints.push({
                        ...
                        segments:
                            preview[k].segments +
                            (preview[k].direct & Direct.diagonal ? 0 : 1),
                    })
                }
                ...
                currentPoints.forEach((info) => {
                    if (
                        bestValue.d === info.d &&
                        bestValue.segments === info.segments
                    ) {
        ...
        for (let y = N; y >= HALF; --y) {
            ...
            for (let k = d; k >= -d; --k) {
                ...
                const currentPoints = [
                    {
                        ...
                        segments:
                            now2[k + 1].segments +
                            (now2[k + 1].direct & Direct.horizontal ? 0 : 1),
                    },
                    {
                        direct: Direct.vertical,
                        d: preview2[k - 1].d + 1,
                        segments:
                            preview2[k - 1].segments +
                            (preview2[k - 1].direct & Direct.vertical ? 0 : 1),
                    },
                ]
                ...
                if (
                    ...
                    y >= 0 &&
                    y < N &&
                    compareFunc(a[aStart + x], b[bStart + y])
                ) {
                    currentPoints.push({
                        direct: Direct.diagonal,
                        d: preview2[k].d,
                        segments:
                            preview2[k].segments +
                            (preview2[k].direct & Direct.diagonal ? 0 : 1),
                    })
                }
        ...
        for (let k = -d; k <= d; ++k) {
            ...
            if (dSum < best.d) {
                ...
                best.segments =
                    now[k].segments +
                    now2[k].segments +
                    (now[k].segments & now2[k].segments ? 0 : 1)
    ...
    return function diff(a, b, compareFunc = (ia, ib) => ia === ib) {
        const ret = {
            removed: [],
            added: [],
        }
